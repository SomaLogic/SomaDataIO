...
library(tidyr)
library(purrr)
set.seed(1)   # set seed for geom::jitter() below so that Git doesn't trigger every time
...
* `example_data`: the original 'SomaScan' file (`example_data.adat`) can
  be found [here](https://github.com/SomaLogic/SomaLogic-Data)
  - an abbreviated, light-weight version containing only the first 10 samples
    can be found at `inst/extdata/example_data10.adat`
...
sample analyses that typical users/clients would perform on 'SomaScan' data.
...
parts or extensions of these techniques. Many variations of the workflows
below exist, however the framework highlights how one could perform standard
_preliminary_ analyses on 'SomaScan' data for:
