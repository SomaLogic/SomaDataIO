...
ggplot() +
  geom_rect(mapping = aes(xmin = 0, xmax = 0.8, ymin = 0, ymax = 0.1),
            fill = "#ecf0de") +
  geom_rect(mapping = aes(xmin = 0.8, xmax = 1.2, ymin = 0, ymax = 0.1),
            fill = "#c3d69b") +
  geom_rect(mapping = aes(xmin = 1.2, xmax = 2, ymin = 0, ymax = 0.1),
            fill = "#ecf0de") +
  labs(title    = "Analysis QC Accuracy Criteria Per SOMAmer Reagent",
       subtitle = "QC Ratio Test By Plate (Aggregate Across Plates)") +
  geom_text(aes(x = 0.4, y = 0.05, label = "Accept"), size = 4) +
  geom_text(aes(x = 1.6, y = 0.05, label = "Accept"), size = 4) +
...
ggplot() +
  geom_rect(mapping = aes(xmin = -2.5, xmax = 0.5, ymin = 0, ymax = 0.1),
            fill = "pink") +
  geom_rect(mapping = aes(xmin = 0.5, xmax = 2.5, ymin = 0, ymax = 0.1),
            fill = "#c3d69b") +
  geom_rect(mapping = aes(xmin = 2.5, xmax = 5.5, ymin = 0, ymax = 0.1),
            fill = "pink") +
  labs(title    = "Standard Normalization Acceptance Criteria Per Sample",
       subtitle = "All Scale Factors") +
  geom_text(aes(x = -1, y = 0.05, label = "FAIL"), size = 4) +
  geom_text(aes(x = 1.5, y = 0.05, label = "PASS"), size = 4) +
  geom_text(aes(x = 4, y = 0.05, label = "FAIL"), size = 4) +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        panel.border = element_blank(),
        axis.text.y = element_blank(),
        ...
        axis.text.x = element_text(size = 12, face = "bold"),
        axis.ticks.length.x = unit(.5, "cm")) +
  scale_x_continuous(breaks = c(-1, 0.5, 1.5, 2.5, 4.0),
                     labels = c("0.25", "0.4", "1.0", "2.5", "4.0"),
                     position = "top") +
  xlab("") +
  ylab("")
```
...
(`NormScale_20`) that fall below the standard accepted range. Filtering
samples based on the default `RowCheck` column will drop these two samples
from `filt_data`.
...
Depending on your data and experiment, it may be reasonable to adjust the
acceptance criteria used for filtering samples. Use your best judgment if using
a non-standard acceptance criteria to filter samples.
...
normalization scale factors with any *endpoint* of interest.
...
feature, `seq.10023.32`, before any transformations are applied.
...
The second transformation to consider applying to the analyte features is a
