...
test_that("adat2eset unit test", {
  ...
  ad <- getFeatureData(sub_adat) %>% data.frame() %>% col2rn("AptName")
  expect_equal(rownames(eSet@featureData@varMetadata), names(ad))
  expect_equal(rownames(eSet@featureData@data), getFeatures(sub_adat))
  ...
  expect_equal(rownames(eSet@assayData$exprs), getFeatures(sub_adat))
